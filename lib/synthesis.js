...
/**
 * Build a condensed data snippet from raw data (max ~4000 chars for prompts)
 */
function buildDataSnippet(rawData) {
  const parts = [];

  // Agentic Search summary (most valuable)
  if (rawData.agenticSearch?.summary) {
    parts.push(`## AI Research Summary\n${rawData.agenticSearch.summary}`);
  }

  // Web search results
  if (rawData.webSearch?.results?.length > 0) {
    const searchItems = rawData.webSearch.results.slice(0, 5)
      .map(r => `- ${r.title}: ${r.snippet || ''}`).join('\n');
    parts.push(`## Recent Web Results\n${searchItems}`);
  }

  // LinkedIn data
  const liData = Object.values(rawData.linkedin || {});
  if (liData.length > 0) {
    parts.push(`## LinkedIn Data\n${JSON.stringify(liData, null, 0).slice(0, 800)}`);
  }

  // Glassdoor data
  const gdData = Object.values(rawData.glassdoor || {});
  if (gdData.length > 0) {
    parts.push(`## Glassdoor Data\n${JSON.stringify(gdData, null, 0).slice(0, 500)}`);
  }

  // GitHub data
  const ghData = Object.values(rawData.github || {});
  if (ghData.length > 0) {
    parts.push(`## GitHub Data\n${JSON.stringify(ghData, null, 0).slice(0, 500)}`);
  }

  // Reddit data
  const rdData = Object.values(rawData.reddit || {});
  if (rdData.length > 0) {
    parts.push(`## Reddit Mentions\n${JSON.stringify(rdData, null, 0).slice(0, 500)}`);
  }

  // Website crawl
  if (rawData.websiteCrawl?.pages?.length > 0) {
    const pageSnippets = rawData.websiteCrawl.pages.slice(0, 3)
...
async function extractPainPoints(companyName, dataSnippet) {
  ...
  return {
    ...
    sources: ['Glassdoor', 'Reddit', 'Web Search'],
  };
}
