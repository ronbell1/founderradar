...
function extractRealPeople(rawData) {
  ...
  if (team.length === 0) {
    for (const val of Object.values(structured)) {
      if (Array.isArray(val) && val.length > 0 && val[0] && (val[0].name || val[0].full_name || val[0].fullName || val[0].title)) {
        team = val;
        break;
      }
    }
  ...
  // Extract from array
  if (team.length > 0) {
    return team.filter(p => p.name || p.full_name || p.fullName).map((p, i) => {
      const personName = p.name || p.full_name || p.fullName;
